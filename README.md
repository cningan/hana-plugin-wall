...
- **需求 · 委托（左栏）**：发布需求帖；卡片下留言认领（昵称选填，可匿名，可互相回复）；「提交成果」一键发布成果并自动绑定该需求（需求随之移入成果区）
- **成果（右栏）**：GitHub 只填仓库名（如 `xiaohong/weather-hana`），自动生成链接、自动从 GitHub API 拉取仓库描述（描述留空时）；手动填描述则优先；成果卡同样可以留言、回复
- **点赞**：需求 / 成果卡片均可点赞，再点一下取消；用浏览器指纹（设备特征哈希）防同一设备反复刷赞
- **留言板**：顶部「💬 留言板」标签页，站内自由讨论，留言可互相回复
- **管理**：页脚「⚙ 管理」输入口令进入管理模式，可编辑 / 删除卡片；编辑和删除都会记录到「📜 操作日志」（仅管理员可见）
...
- 存储：`data.json`（帖子）+ `logs.json`（操作日志）+ `likes.json`（点赞指纹）+ `wall.json`（留言板），均为原子写入 + 线程锁；换机器 = 拷贝这些文件
